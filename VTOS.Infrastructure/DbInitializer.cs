...
public static class DbInitializer
{
    ...
    public static async Task SeedAsync(VTOSDbContext db)
    {
        ...
        db.PayoutRecords.AddRange(bundle.PayoutRecords);
        db.WalletWithdrawalRequests.AddRange(bundle.WithdrawalRequests);
    ...
    private static IEnumerable<Role> BuildRoles(DateTime now)
    {
        return
        [
            new Role { Id = RoleAdmin,    RoleName = "Admin",           Description = "System administrator.",                    IsSystemRole = true, CreatedAt = now },
            new Role { Id = RoleParent,   RoleName = "Parent",          Description = "Parent user linked to student records.",    IsSystemRole = true, CreatedAt = now },
            new Role { Id = RoleSchool,   RoleName = "School",          Description = "School manager account.",                  IsSystemRole = true, CreatedAt = now },
            new Role { Id = RoleProvider, RoleName = "Provider",        Description = "Uniform provider account.",                IsSystemRole = true, CreatedAt = now },
            new Role { Id = RoleTeacher,  RoleName = "HomeroomTeacher", Description = "Homeroom teacher created for seeded classes.", IsSystemRole = true, CreatedAt = now }
        ];
    }
    ...
    private static SeedBundle BuildSeedBundle(DateTime now, string academicYear, string passwordHash)
    {
        ...
        {
            Id           = AdminUserId,
            FullName     = "Nguyễn Đình Quản Trị",
            Email        = "khoindqde180179@fpt.edu.vn",
        ...
        var teacherCounter = new SequenceCounter();
        var parentCounter  = new SequenceCounter();
        ...
        CreateClassroomsAndTeachers(bundle, schoolContexts, now, academicYear, passwordHash, teacherCounter);
        CreateStudentsAndParents(bundle, schoolContexts, now, academicYear, passwordHash, parentCounter);
        CreateTeacherReports(bundle, schoolContexts, now);
        CreateOrdersPaymentsAndFeedback(bundle, schoolContexts, now);
        CreateWithdrawalRequests(bundle, providerContexts, now);
    ...
    private static List<ProviderContext> CreateProviders(SeedBundle bundle, DateTime now, string passwordHash)
    {
        var blueprints = new[]
        {
            new ProviderBlueprint("provider-hoanggia",  "Công ty May Mặc Hoàng Gia",  "Nguyễn Minh Tuấn",  "khoikhung670@gmail.com",         "0905123456", "Lô C2 KCN Hòa Khánh, Liên Chiểu, Đà Nẵng",       "0401987654", "Giám đốc điều hành",  "VCB",  "Vietcombank",  "0491000234567",  "CONG TY MAY MAC HOANG GIA"),
            new ProviderBlueprint("provider-sontra",    "Đồng Phục Sơn Trà",          "Võ Thị Lan Anh",    "khoikhung670+ncc2@gmail.com",    "0935789012", "78 Ngô Quyền, Sơn Trà, Đà Nẵng",                 "0402456789", "Giám đốc kinh doanh", "TCB",  "Techcombank",  "19035678905678", "DONG PHUC SON TRA"),
            new ProviderBlueprint("provider-lienchieu", "Xưởng May Liên Chiểu",       "Phạm Đức Long",     "khoikhung670+ncc3@gmail.com",    "0769456789", "152 Nguyễn Lương Bằng, Liên Chiểu, Đà Nẵng",     "0403123456", "Quản lý xưởng",       "BIDV", "BIDV",         "31410001234567", "XUONG MAY LIEN CHIEU"),
            new ProviderBlueprint("provider-nhatnam",   "May Mặc Nhật Nam",            "Đoàn Thị Hương",    "khoikhung670+ncc4@gmail.com",    "0912345678", "45 Trần Cao Vân, Thanh Khê, Đà Nẵng",             "0404567890", "Giám đốc",            "MB",   "MB Bank",      "0706100789012",  "MAY MAC NHAT NAM"),
            new ProviderBlueprint("provider-daiphuc",   "Xưởng May Đại Phúc",          "Trần Công Minh",    "khoikhung670+ncc5@gmail.com",    "0987654321", "112 Nguyễn Tất Thành, Hải Châu, Đà Nẵng",        "0405678901", "Quản lý sản xuất",   "ACB",  "ACB",          "1234567890123",  "XUONG MAY DAI PHUC"),
        };

        var contexts = new List<ProviderContext>();

        for (var bpIndex = 0; bpIndex < blueprints.Length; bpIndex++)
        {
            var blueprint    = blueprints[bpIndex];
            var registeredAt = now.AddMonths(-(11 - bpIndex)); // providers 0-4 → 11,10,9,8,7 months ago
            ...
            bundle.Providers.Add(new Provider
            {
                Id                      = providerId,
        ...
        string passwordHash)
    {
        var blueprints = new[]
        {
            new SchoolBlueprint("school-thpt",           "Trường THPT Phan Châu Trinh",    "THPT",     "154 Lê Lợi, Hải Châu, Đà Nẵng",              "khoikhung291@gmail.com",              "02363822367", "Nguyễn Thị Thanh Hà",  "Hiệu trưởng", 10, 12, 8, 18, 24),
            new SchoolBlueprint("school-thcs",           "Trường THCS Nguyễn Huệ",         "THCS",     "62 Nguyễn Chí Thanh, Hải Châu, Đà Nẵng",      "khoikhung291+truong2@gmail.com",      "02363823456", "Trần Văn Minh",         "Hiệu trưởng", 6,  9,  4, 16, 22),
            new SchoolBlueprint("school-tieuhoc",        "Trường Tiểu học Lê Quý Đôn",     "Tiểu học", "85 Hải Phòng, Thanh Khê, Đà Nẵng",            "khoikhung291+truong3@gmail.com",      "02363778999", "Lê Thị Bích Ngọc",     "Hiệu trưởng", 1,  5,  4, 14, 20),
            new SchoolBlueprint("school-tranphu",        "Trường THPT Trần Phú",           "THPT",     "20 Phan Châu Trinh, Hải Châu, Đà Nẵng",       "khoikhung291+truong4@gmail.com",      "02363811111", "Ngô Hải Minh",          "Hiệu trưởng", 10, 12, 5, 18, 24),
            new SchoolBlueprint("school-lytutrongthcs",  "Trường THCS Lý Tự Trọng",        "THCS",     "50 Hoàng Diệu, Hải Châu, Đà Nẵng",            "khoikhung291+truong5@gmail.com",      "02363822222", "Phạm Thị Xuân",         "Hiệu trưởng", 6,  9,  3, 16, 22),
        };
        ...
        for (var index = 0; index < blueprints.Length; index++)
        {
            ...
            var registeredAt = now.AddMonths(-(10 - index)); // schools 0-4 → 10,9,8,7,6 months ago
            var schoolId     = StableGuid($"{blueprint.Key}:school");
            var managerId    = StableGuid($"{blueprint.Key}:manager");
            var sizeChartId  = StableGuid($"{blueprint.Key}:sizechart");

            bundle.Schools.Add(new School
            {
                Id                      = schoolId,
                SchoolName              = blueprint.SchoolName,
                Level                   = blueprint.Level,
                LogoURL                 = GetSchoolLogoUrl(blueprint.Key),
            ...
            bundle.SizeCharts.Add(new SizeChart
            {
                Id          = sizeChartId,
    ...
    private static string GetSchoolLogoUrl(string key) => key switch
    {
        "school-thpt"    => "https://media.vtos.homes/seeds/schools/school-thpt/logo.png",
        "school-thcs"    => "https://media.vtos.homes/seeds/schools/school-thcs/logo.png",
        "school-tieuhoc" => "https://media.vtos.homes/seeds/schools/school-tieuhoc/logo.png",
        "school-tranphu"       => "https://placehold.co/200x200?text=TranPhu",
        "school-lytutrongthcs" => "https://placehold.co/200x200?text=LyTuTrong",
        _ => $"https://placehold.co/200x200?text={key}"
    };
    ...
    private static void CreateSizeChartDetails(SeedBundle bundle, SchoolBlueprint blueprint, Guid sizeChartId, DateTime now)
    {
        ...
        for (var index = 0; index < sizes.Length; index++)
        {
            ...
            bundle.SizeChartMeasurements.AddRange(
            [
                new SizeChartMeasurement { Id = StableGuid($"{detailId}:height"), SizeChartDetailId = detailId, FieldKey = "height", DisplayName = "Chiều cao", Unit = "cm", MinCm = heightMin, MaxCm = heightMax, CreatedAt = now },
                new SizeChartMeasurement { Id = StableGuid($"{detailId}:chest"),  SizeChartDetailId = detailId, FieldKey = "chest",  DisplayName = "Vòng ngực", Unit = "cm", MinCm = chestMin,  MaxCm = chestMax,  CreatedAt = now },
                new SizeChartMeasurement { Id = StableGuid($"{detailId}:waist"),  SizeChartDetailId = detailId, FieldKey = "waist",  DisplayName = "Vòng eo",   Unit = "cm", MinCm = waistMin,  MaxCm = waistMax,  CreatedAt = now }
            ]);
        }
    }
    ...
    private static void CreateOutfitsForSchool(SeedBundle bundle, SchoolContext context, DateTime now)
    {
        ...
        for (var index = 0; index < outfitBlueprints.Length; index++)
        {
            ...
            var imageUrl  = $"https://placehold.co/400x400?text={context.Blueprint.Key}-{blueprint.Code}";

            bundle.Outfits.Add(new Outfit
            {
                Id            = outfitId,
    ...
    private static void CreateContractsForSchool(SeedBundle bundle, SchoolContext context, DateTime now)
    {
        ...
        if (groupedOutfits.TryGetValue(context.PrimaryProvider.ProviderId, out var primaryOutfits))
        {
            foreach (var outfit in primaryOutfits)
            {
        ...
        if (groupedOutfits.TryGetValue(context.SecondaryProvider.ProviderId, out var secondaryOutfits))
        {
            foreach (var outfit in secondaryOutfits)
            {
                var contractPrice = Math.Round(outfit.BasePrice * 0.70m, 0);
                context.ProviderCostByOutfitId[outfit.OutfitId] = contractPrice;
                bundle.ContractItems.Add(new ContractItem
                {
    ...
    private static void CreateSemesterPublications(SeedBundle bundle, SchoolContext context, DateTime now, string academicYear)
    {
        ...
        bundle.SemesterPublications.AddRange(
        [
        ...
        bundle.SemesterPublicationProviders.AddRange(
        [
            new SemesterPublicationProvider { Id = StableGuid($"{activeId}:provider:{context.PrimaryProvider.ProviderId}"),   SemesterPublicationID = activeId,  ProviderID = context.PrimaryProvider.ProviderId,   ContractID = context.ActivePrimaryContractId,   Status = SemPublicationProviderStatus.Active,     CreatedAt = now.AddDays(-25) },
            new SemesterPublicationProvider { Id = StableGuid($"{activeId}:provider:{context.SecondaryProvider.ProviderId}"), SemesterPublicationID = activeId,  ProviderID = context.SecondaryProvider.ProviderId, ContractID = context.ActiveSecondaryContractId, Status = SemPublicationProviderStatus.Active,     CreatedAt = now.AddDays(-25) },
            new SemesterPublicationProvider { Id = StableGuid($"{closedId}:provider:{context.PrimaryProvider.ProviderId}"),  SemesterPublicationID = closedId,  ProviderID = context.PrimaryProvider.ProviderId,   ContractID = context.ActivePrimaryContractId,   Status = SemPublicationProviderStatus.Active,     CreatedAt = now.AddDays(-170) },
            new SemesterPublicationProvider { Id = StableGuid($"{draftId}:provider:{context.PrimaryProvider.ProviderId}"),   SemesterPublicationID = draftId,   ProviderID = context.PrimaryProvider.ProviderId,   ContractID = context.ActivePrimaryContractId,   Status = SemPublicationProviderStatus.Active,     CreatedAt = now.AddDays(-4) },
            new SemesterPublicationProvider { Id = StableGuid($"{draftId}:provider:{context.SecondaryProvider.ProviderId}"), SemesterPublicationID = draftId,   ProviderID = context.SecondaryProvider.ProviderId, ContractID = context.ActiveSecondaryContractId, Status = SemPublicationProviderStatus.Suspended,  SuspendedAt = now.AddDays(-1), SuspendReason = "Chưa chốt giá danh mục hè.", CreatedAt = now.AddDays(-4) }
        ]);
    }
    ...
    private static void CreateClassroomsAndTeachers(
        ...
        string academicYear,
        string passwordHash,
        SequenceCounter teacherCounter)
    {
        foreach (var school in schoolContexts)
        {
            ...
            for (var grade = school.Blueprint.StartGrade; grade <= school.Blueprint.EndGrade; grade++)
            {
                for (var section = 1; section <= school.Blueprint.ClassesPerGrade; section++)
                {
                    ...
                    var teacherEmail      = teacherCounter.NextEmail("t");
                    var teacherRegMonths  = StableInt($"{school.Blueprint.Key}:teacher:{classCode}:reg", 2, 9);
                    var teacherRegistered = now.AddMonths(-teacherRegMonths);
        ...
        foreach (var school in schoolContexts)
        {
            foreach (var classContext in school.Classes)
            {
                ...
                for (var studentIndex = 1; studentIndex <= studentCount; studentIndex++)
                {
                    ...
                    var birthDay   = StableInt($"{studentKey}:day", 1, DateTime.DaysInMonth(Math.Max(1, birthYear), birthMonth));
                    var dob        = new DateTime(Math.Max(1, birthYear), birthMonth, birthDay, 0, 0, 0, DateTimeKind.Utc);
                    ...
                    if (shouldLinkParent)
                    {
                        var householdKey = BuildHouseholdKey(studentKey, studentIndex, householdKeysInClass, school.Blueprint.Key, classContext.ClassName);
                        householdKeysInClass.Add(householdKey);

                        if (!households.TryGetValue(householdKey, out parent))
                        {
                            parent = CreateParentContext(householdKey, childId, isMale, passwordHash, now, parentCounter);
                    ...
                    var height = hasMeasurements ? ResolveHeightForGrade(classContext.Grade, school.Blueprint.Level, studentKey) : 0;
                    var weight = hasMeasurements ? ResolveWeightForGrade(classContext.Grade, school.Blueprint.Level, studentKey) : 0;

                    bundle.ChildProfiles.Add(new ChildProfile
                    {
                        Id            = childId,
                    ...
                    bundle.StudentDataImports.Add(new StudentDataImport
                    {
    ...
    private static void CreateTeacherReports(SeedBundle bundle, IReadOnlyList<SchoolContext> schoolContexts, DateTime now)
    {
        foreach (var school in schoolContexts)
        {
            for (var index = 0; index < school.Classes.Count; index++)
            {
                ...
                {
                    ClassScenario.ParentGap => (TeacherReportType.OrderCoverage, TeacherReportStatus.Submitted,
                        ...
                        (string?)null),
                    ClassScenario.MeasurementGap => (TeacherReportType.QualityIssue, TeacherReportStatus.Submitted,
                        ...
                        (string?)null),
                    ClassScenario.OrderGap => (TeacherReportType.OrderCoverage, TeacherReportStatus.Reviewed,
                    ...
                    _ => (TeacherReportType.General, TeacherReportStatus.Reviewed,
        ...
        foreach (var school in schoolContexts)
        {
            foreach (var classContext in school.Classes)
            {
                foreach (var student in classContext.Students.Where(x => x.ParentLinked))
                {
                    ...
                    var bucket = orderOrdinal % 12;
                    var status = ResolveStatusFromBucket(bucket, student.ChildId);
                    orderOrdinal++;

                    var providerId = ResolveProviderIdForOrder(school, student.ChildId, status);
                    var publicationId = status is OrderStatus.Delivered or OrderStatus.Refunded
                    ...
                    var orderDate      = ResolveOrderDateFromBucket(now, bucket, orderOrdinal);
                    ...
                    var needsShipping  = status is OrderStatus.ReadyToShip or OrderStatus.Shipped or OrderStatus.Delivered;
                    var shippingCompany = needsShipping ? Pick($"{orderId}:shipper", shippingCompanies) : null;
                    var trackingCode   = shippingCompany != null ? $"VTOS-{GetCompactCode(school.Blueprint.Key)}-{orderOrdinal:0000}" : null;
                    ...
                    for (var itemIndex = 0; itemIndex < chosenOutfits.Count; itemIndex++)
                    {
                        ...
                        providerPayout += GetProviderCost(school.ProviderCostByOutfitId, outfit.OutfitId) * quantity;
                    ...
                    CreatePaymentTransactions(bundle, school, student, providerId, orderId, status,
                        totalAmount, providerPayout, orderDate, recipientName);
        ...
        string recipientName)
    {
        ...
        // Pending: link only, no payment yet
        if (status == OrderStatus.Pending)
        {
            bundle.PaymentTransactions.Add(new PaymentTransaction
            {
                Id                   = StableGuid($"{orderId}:tx:pending"),
                OrderID              = orderId,
                WalletID             = null,
                PaymentLinkId        = $"payos-{orderId:N}",
        ...
        // Paid (completed) for non-cancelled orders
        if (status != OrderStatus.Cancelled)
        {
            bundle.PaymentTransactions.Add(new PaymentTransaction
            {
                Id                   = StableGuid($"{orderId}:tx:order-payment"),
                OrderID              = orderId,
                WalletID             = null,
                PaymentLinkId        = $"payos-{orderId:N}",
                TransactionType      = TransactionType.OrderPayment,
        ...
        if (status == OrderStatus.Refunded)
        {
            ...
            bundle.PaymentTransactions.Add(new PaymentTransaction
            {
                Id                   = StableGuid($"{orderId}:tx:refund"),
                OrderID              = orderId,
                WalletID             = parentWallet.Id,
                TransactionType      = TransactionType.Refund,
                GatewayType          = PaymentGatewayType.Other,
                ...
                Description          = $"Hoàn tiền cho phụ huynh đơn {orderId:N}.",
                CreatedAt            = orderDate.AddDays(2)
            });
        }

        // Delivered: escrow release → provider payout (net) + platform fee (1%)
        if (status == OrderStatus.Delivered && providerPayout > 0)
        {
            var grossAmount  = providerPayout;
            var feeAmount    = Math.Round(grossAmount * PlatformFeeConstants.OrderFeeRate, 2);
            var netAmount    = grossAmount - feeAmount;
            var payoutDate   = orderDate.AddDays(9);
            var payoutRecordId = StableGuid($"{orderId}:payout-record");

            bundle.PayoutRecords.Add(new PayoutRecord
            {
                Id                = payoutRecordId,
                ProviderID        = providerId,
                OrderID           = orderId,
            ...
            bundle.PaymentTransactions.Add(new PaymentTransaction
            {
                Id                   = StableGuid($"{orderId}:tx:escrow-release"),
                OrderID              = orderId,
                WalletID             = null,
                ...
                TransactionStatus    = PaymentStatus.Completed,
                Amount               = grossAmount,
                TransactionTimestamp = payoutDate,
                Description          = $"Giải ngân {grossAmount:N0}đ từ escrow cho đơn {orderId:N}.",
                CreatedAt            = payoutDate
            });

            bundle.PaymentTransactions.Add(new PaymentTransaction
            {
                Id                   = StableGuid($"{orderId}:tx:provider-payout"),
                OrderID              = orderId,
                ...
                Amount               = netAmount,
                TransactionTimestamp = payoutDate,
                Description          = $"Ghi có ví nhà cung cấp {netAmount:N0}đ (sau phí {PlatformFeeConstants.OrderFeeRate * 100:0}%).",
                CreatedAt            = payoutDate
            });

            bundle.PaymentTransactions.Add(new PaymentTransaction
            {
                Id                   = StableGuid($"{orderId}:tx:platform-fee"),
                OrderID              = orderId,
                WalletID             = null,
                PayoutRecordID       = payoutRecordId,
                ...
                Amount               = feeAmount,
                TransactionTimestamp = payoutDate,
                Description          = $"Phí nền tảng {PlatformFeeConstants.OrderFeeRate * 100:0}% cho đơn {orderId:N}: {feeAmount:N0}đ.",
                CreatedAt            = payoutDate
            });
        }
    }
    ...
    // ──────────────────────────────────────────────────────────────
    // Withdrawal Requests (2 per provider, status Paid)
    // ──────────────────────────────────────────────────────────────

    private static void CreateWithdrawalRequests(SeedBundle bundle, List<ProviderContext> providers, DateTime now)
    {
        foreach (var provider in providers)
        {
            for (var i = 1; i <= 2; i++)
            {
                var monthsAgo   = i == 1 ? 4 : 7;
                var requestedAt = now.AddMonths(-monthsAgo).AddDays(StableInt($"{provider.ProviderId}:wr{i}:day", 1, 10));
                var amount      = StableInt($"{provider.ProviderId}:wr{i}:amount", 200, 800) * 1000m;
                var feeRate     = PlatformFeeConstants.WithdrawalFeeRate;
                var feeAmount   = Math.Round(amount * feeRate, 2);
                var netAmount   = amount - feeAmount;
                var requestId   = StableGuid($"{provider.ProviderId}:wr{i}");
                var approvedAt  = requestedAt.AddDays(2);
                var paidAt      = approvedAt.AddDays(1);

                bundle.WithdrawalRequests.Add(new WalletWithdrawalRequest
                {
                    Id          = requestId,
                    ...
                    PaidAt      = paidAt,
                    AdminNote   = "Đã xử lý và chuyển khoản ngân hàng cho nhà cung cấp."
                });

                // Net amount transferred to provider bank (off-platform, no wallet credit)
                bundle.PaymentTransactions.Add(new PaymentTransaction
                {
                    Id                   = StableGuid($"{requestId}:tx:withdrawal"),
                    WalletID             = null,
                    ...
                    Description          = $"Chuyển khoản {netAmount:N0}đ cho NCC (yêu cầu {requestId:N}).",
                    CreatedAt            = paidAt
                });

                // Platform withdrawal fee (admin revenue)
                bundle.PaymentTransactions.Add(new PaymentTransaction
                {
                    Id                   = StableGuid($"{requestId}:tx:withdrawal-fee"),
                    WalletID             = null,
                    ...
                    Amount               = feeAmount,
                    TransactionTimestamp = paidAt,
                    Description          = $"Phí rút tiền {feeRate * 100:0}% cho yêu cầu {requestId:N}: {feeAmount:N0}đ.",
                    CreatedAt            = paidAt
                });
            }
        }
    ...
    private static void UpdateWalletBalances(SeedBundle bundle)
    {
        ...
        {
            if (!walletLookup.TryGetValue(tx.WalletID!.Value, out var wallet))
                continue;

            wallet.Balance += wallet.OwnerType switch
            {
                WalletOwnerType.Provider => tx.TransactionType == TransactionType.ProviderPayout ? tx.Amount : 0m,
                WalletOwnerType.Parent   => tx.TransactionType == TransactionType.Refund ? tx.Amount : 0m,
                _                        => 0m
            };
        }

        // Approved/paid withdrawals debit from provider wallet
        foreach (var wr in bundle.WithdrawalRequests.Where(x => x.Status is "Approved" or "Paid"))
        {
            if (walletLookup.TryGetValue(wr.WalletID, out var wallet))
                wallet.Balance = Math.Max(0m, wallet.Balance - wr.Amount);
        }
        ...
        SequenceCounter parentCounter)
    {
        ...
        var parentRegMonths  = StableInt($"{householdKey}:reg-month", 1, 7);
        var parentRegistered = now.AddMonths(-parentRegMonths);

        var user = new User
        {
            Id           = parentId,
            FullName     = parentName,
            Email        = parentCounter.NextEmail("p"),
            PasswordHash = passwordHash,
            Phone        = phone,
            Avatar       = "avatar.jpg",
            RoleID       = RoleParent,
    ...
    private static OrderStatus ResolveStatusFromBucket(int bucket, Guid childId)
    {
        return bucket switch
        {
            0 => StableInt($"{childId}:st-b0", 0, 1) == 0 ? OrderStatus.Pending      : OrderStatus.Paid,
            1 => StableInt($"{childId}:st-b1", 0, 1) == 0 ? OrderStatus.Accepted     : OrderStatus.Confirmed,
            2 => StableInt($"{childId}:st-b2", 0, 1) == 0 ? OrderStatus.Processed    : OrderStatus.InProduction,
            3 => StableInt($"{childId}:st-b3", 0, 1) == 0 ? OrderStatus.ReadyToShip  : OrderStatus.Shipped,
            _ => StableInt($"{childId}:st-b{bucket}", 0, 99) < 80 ? OrderStatus.Delivered : OrderStatus.Refunded
        };
    }

    private static DateTime ResolveOrderDateFromBucket(DateTime now, int bucket, int ordinal)
    {
        // Place the order in the correct month, with intra-month jitter
        var daysBack = bucket * 30 + 3 + (ordinal / 12) % 22;
        return now.AddDays(-daysBack);
    }
    ...
    private static decimal GetProviderCost(Dictionary<Guid, decimal> costMap, Guid outfitId)
        => costMap.TryGetValue(outfitId, out var value) ? value : 0m;

    private static int GetLinkThreshold(ClassScenario scenario) => scenario switch
    {
    ...
    private static int GetMeasurementThreshold(ClassScenario scenario) => scenario switch
    {
        ClassScenario.Healthy        => 92,
    ...
    private static ClassScenario ResolveScenario(int classOrdinal) => classOrdinal switch
    {
    ...
    private static int SizeDistance(string size, bool hasMeasurements)
    {
        if (!hasMeasurements)
        {
            return size switch { "M" => 0, "S" => 1, "L" => 2, "XS" => 2, _ => 3 };
        }
        return size switch { "S" => 1, "M" => 0, "L" => 1, "XL" => 2, "XS" => 2, _ => 3 };
    }
    ...
    private static string Pick(string key, string[] values)
        => values[StableInt(key, 0, values.Length - 1)];
    ...
    private sealed class SequenceCounter
    {
        ...
        public string NextEmail(string tag)
        {
            var n = _value++;
            return n % 2 == 0
                ? $"auknjsh+{tag}{n:D4}@gmail.com"
                : $"wibulord32+{tag}{n:D4}@gmail.com";
        }
    }
    ...
    private sealed class SeedBundle
    {
        ...
        public List<PayoutRecord>                PayoutRecords                { get; } = new();
        public List<WalletWithdrawalRequest>     WithdrawalRequests           { get; } = new();
    ...
    private sealed record SchoolBlueprint(
        string Key, string SchoolName, string Level, string Address, string Email, string Phone,
        string RepresentativeName, string RepresentativeTitle,
        int StartGrade, int EndGrade, int ClassesPerGrade, int MinStudents, int MaxStudents);

    private sealed record ProviderBlueprint(
        string Key, string ProviderName, string ContactPersonName, string Email, string Phone, string Address,
        string TaxCode, string RepresentativeTitle,
        string BankCode, string BankName, string BankAccountNumber, string BankAccountName);

    private sealed record OutfitBlueprint(
        string Code, string Name, string Description, string MaterialType,
        string Color, decimal BasePrice, OutfitType OutfitType);

    private enum ClassScenario { Healthy, ParentGap, MeasurementGap, OrderGap, Mixed }
}
