...
public class GetCategoriesQueryHandler
{
    ...
    public async Task<CategoryListResponse> HandleAsync(GetCategoriesQuery query, CancellationToken ct = default)
    {
        var legacyOutfitCounts = await _context.Outfits
            .AsNoTracking()
            .Where(o => !o.IsDeleted && o.IsAvailable && !o.OutfitCategories.Any())
            .GroupBy(o => o.OutfitType)
            .Select(g => new { OutfitType = g.Key, Count = g.Count() })
            .ToDictionaryAsync(x => x.OutfitType, x => x.Count, ct);
        ...
        var categories = await _context.Categories
            ...
            .Select(c => new
            {
                c.Id,
                c.CategoryName,
                LinkedOutfitCount = c.OutfitCategories.Count(oc => oc.Outfit.IsAvailable)
            })
            .ToListAsync(ct);
